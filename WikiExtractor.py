...
#
#
#  Chad Liu modify birthday template expansion version @ 2013
#
#
# =============================================================================
...
def clean(text):
    ...
    # birthday template expansion
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|([^}}]*?)\|\|\|([^}}]*{{[^}}]*}}|[^}}]*)?}}', r'\1\2~', text) # {{bd|1953年|3月16日|||Stallman}}
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|([^}}]*?)\|([^}}]{1,10}?)\|([^}}]*?)\|([^}}]*{{[^}}]*}}|[^}}]*)?}}', r'\1\2~\3\4', text) # {{bd|1866年|11月12日|1925年|3月12日|catIdx=S孫}}
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|\|([^}}]*?)\|([^}}]*{{[^}}]*}}|[^}}]*)?}}', r'\1~\2', text) # {{bd|1640年||1715年|catIdx=Pu,蒲}}
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|([^}}]{1,*}?)\|([^}}]{1,*}?)\|([^}}]{1,*}?)}}', r'\1\2~\3\4',text) # {{bd|1882年|7月2日|1962年|9月21日}}
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|([^}}]*?)\|([^}}]*{{[^}}]*}}|[^}}]*)?}}', r'\1\2~',text) #{{bd|1938年|1月10日|catIdx=G高}}
    text = re.sub(r'(?i){{bd\|([^}}]{1,10}?)\|([^}}]*?)}}', r'\1\2~',text) #{{bd|1939年|7月26日}}
    text = re.sub(r'(?i){{bd\|([^}}]*)\|\|}}', r'\1~',text) # {{bd|1994年6月20日||}}
    text = re.sub(r'(?i){{bd\|\|([^}}]*)}}', r'\1~',text) #{{Bd||1977年4月2日}}
    text = re.sub(r'(?i){{bd\|([^}}]*)\|[^}}]*}}', r'\1~',text) # {{bd|1990年10月16日|catIdx=Sato Amina}}
    text = re.sub(r'(?i){{birth date and age\|([0-9]{1,4})\|([0-9]{1,2})\|([0-9]{1,2})}}', r'\1\2\3~',text) #{{birth date and age|1985|12|24}}
